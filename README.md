...
Each click adjusts stock by a single unit, calling a dedicated update route for that product. This keeps the validation logic simple and the displayed state always in sync with the database.

---

### 3. Negative Stock Prevention

The application prevents inventory quantities from becoming negative. This is enforced in two layers:
...
Invalid updates are rejected and never saved to the database. This check runs identically on both the dashboard's form route and the JSON API route.

#### Frontend Validation (UX only)

When a product's stock reaches zero, the minus button is replaced with one that triggers a popup warning instead of submitting a form:

```text
❌ Negative stock is not allowed!
```

This avoids a round trip to the server just to show the user why the action is blocked. The backend check above is what actually protects data integrity — the popup is a convenience layer on top of it.
...
Products with stock less than or equal to 3 (and greater than 0) are automatically classified as Low Stock. Products at exactly 0 are classified separately as Out of Stock.
...
Negative stock values are never saved by either the dashboard form route or the JSON API route.
...
* `price` and the initial `stock` value are not currently checked for positivity at product-creation time — only later stock *updates* are floored at zero. Adding a Pydantic field validator (or a manual range check) would close this gap.
* Concurrent stock updates from multiple simultaneous requests are not guarded against (no row locking or optimistic concurrency check). This is acceptable at single-administrator scale but would need addressing before multi-user use.
* The application currently drops and recreates all database tables on every startup (see `main.py`'s lifespan handler), so data does not persist across server restarts — only within a single run. This was convenient during development/demo but should be removed for any persistent deployment.
...
* Validation on price/stock positivity at creation time
...
```bash
git clone https://github.com/03uzma/Inventory-Management-System1.git
cd Inventory-Management-System1
```
...
```bash
python -m pip install fastapi uvicorn jinja2 sqlalchemy python-multipart
```
...
![Dashboard when products are added](Screenshots/Products-added.png)
